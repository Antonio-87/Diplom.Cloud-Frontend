...
function* handleGetTokenRefreshRequestSaga(): Generator<
...
> {
  ...
  if (
    validatedTokenState.data.expiration &&
    !(validatedTokenState.data.expiration <= Date.now())
  ) {
    return;
  }
  try {
...
function* watchGetTokenRefreshRequestSaga(): Generator<ForkEffect> {
  yield takeLatest(
    getTokenRefreshRequest.type,
    handleGetTokenRefreshRequestSaga
  );
}
...
export {
  handleGetTokenRefreshRequestSaga,
  watchClearTokenData,
  watchGetTokenRefreshRequestSaga,
};
