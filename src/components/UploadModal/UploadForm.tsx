...
const isRcFile = (data: unknown): data is RcFile => {
  ...
  return (
    <Form>
      ...
      <Form.Item
        label="Path"
        validateStatus={
          (((path !== "" && !pathRegExp.test(path)) || path.includes("//")) &&
            "error") ||
          undefined
        }
        help={
          (((path !== "" && !pathRegExp.test(path)) || path.includes("//")) &&
            "Incorrect path format. Forbidden symbols: '.', '\\'. Example: 'home/folder/path/'.") ||
          undefined
        }
        rules={[
          {
      ...
      >
        <Input
          placeholder='"example/folder/" or empty for saving in the current folder'
          value={path}
          onChange={(e) => {
