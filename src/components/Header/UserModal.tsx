...
const UserModal = ({ open, onClose }: UserModalProps): JSX.Element => {
  ...
  const handleOk = (): void => {
    ...
    dispatch(
      updateUserRequest({
        updateFunction: async (token) =>
          await updateUser(user.id, token, states),
        callback: () => {
          onClose();
          resetUserEditStates();
        },
        ...
        <div key="delete" style={{ width: "9em" }}>
          <PopoverWrapper
            ...
            onConfirmHandler={() => {
              const user = userState.user;
              if (user && accessToken) {
                void deleteUser(user.id, accessToken).then(() => {
                  localStorage.clear();
                  handleCancel();
