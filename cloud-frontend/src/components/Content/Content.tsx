...
const checkSameFolder = (
  ...
  pathList.some(
    (path) =>
      path.replace(currentPath, "").split("/")[0] ===
      checkedPath.replace(currentPath, "").split("/")[0]
  );
...
const Content = ({
  ...
  files.forEach((file) => {
    ...
    if (
      ...
      !checkSameFolder(currentPath, file.path, pathList)
    ) {
      ...
      fileList.push(
        <File
          ...
          onSetFileClicked={setFileClicked}
        />
      );
    }
  });
